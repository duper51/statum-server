...
use \Exception;
use \PDO;
use Me\Models\Me\Models\Services as ChildServices;
use Me\Models\Me\Models\ServicesQuery as ChildServicesQuery;
use Me\Models\Me\Models\Map\ServicesTableMap;
 ...
 * @method     ChildServicesQuery orderByPortnum($order = Criteria::ASC) Order by the portnum column
 * @method     ChildServicesQuery orderByServicename($order = Criteria::ASC) Order by the servicename column
 *
 * @method     ChildServicesQuery groupById() Group by the id column
 * @method     ChildServicesQuery groupByPortnum() Group by the portnum column
 * @method     ChildServicesQuery groupByServicename() Group by the servicename column
 *
 ...
 * @method     ChildServices findOneByPortnum(int $portnum) Return the first ChildServices filtered by the portnum column
 * @method     ChildServices findOneByServicename(string $servicename) Return the first ChildServices filtered by the servicename column *
 ...
 * @method     ChildServices requireOneByPortnum(int $portnum) Return the first ChildServices filtered by the portnum column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 * @method     ChildServices requireOneByServicename(string $servicename) Return the first ChildServices filtered by the servicename column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildServices[]|ObjectCollection findByPortnum(int $portnum) Return ChildServices objects filtered by the portnum column
 * @method     ChildServices[]|ObjectCollection findByServicename(string $servicename) Return ChildServices objects filtered by the servicename column
...
abstract class ServicesQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Me\Models\Me\Models\Base\ServicesQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Me\\Models\\Me\\Models\\Services', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, portnum, servicename FROM services WHERE id = :p0';
        try {
    ...
    /**
     * Filter the query on the portnum column
     *
     * Example usage:
     * <code>
     * $query->filterByPortnum(1234); // WHERE portnum = 1234
     * $query->filterByPortnum(array(12, 34)); // WHERE portnum IN (12, 34)
     * $query->filterByPortnum(array('min' => 12)); // WHERE portnum > 12
     * </code>
     *
     * @param     mixed $portnum The value to use as filter.
     *              Use scalar values for equality.
     *              Use array values for in_array() equivalent.
     *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     ...
     * @return $this|ChildServicesQuery The current query, for fluid interface
     */
    public function filterByPortnum($portnum = null, $comparison = null)
    {
        if (is_array($portnum)) {
            $useMinMax = false;
            if (isset($portnum['min'])) {
                $this->addUsingAlias(ServicesTableMap::COL_PORTNUM, $portnum['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($portnum['max'])) {
                $this->addUsingAlias(ServicesTableMap::COL_PORTNUM, $portnum['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(ServicesTableMap::COL_PORTNUM, $portnum, $comparison);
    }

    /**
     * Filter the query on the servicename column
     *
     * Example usage:
     * <code>
     * $query->filterByServicename('fooValue');   // WHERE servicename = 'fooValue'
     * $query->filterByServicename('%fooValue%', Criteria::LIKE); // WHERE servicename LIKE '%fooValue%'
     * </code>
     *
    ...
    public function filterByServicename($servicename = null, $comparison = null)
    {
        ...
        return $this->addUsingAlias(ServicesTableMap::COL_SERVICENAME, $servicename, $comparison);
    }
